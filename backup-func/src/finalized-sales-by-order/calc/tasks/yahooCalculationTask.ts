 ...
 */
//-------------------------------------------------------------------------------
export const yahooCalculationTask: CalculationTask<IFinalizedSalesByOrder> = async (finalizedShippingResult: IFinalizedShipping[], costMaster: ICostMaster[]) => {
  ...
  const yahooList = finalizedShippingResult.filter((i) => i.salesChannel === SalesChannel.Yahoo);
  ...
  const result = await Promise.allSettled(
    ...
    yahooList.map(async (fs): Promise<IFinalizedSalesByOrder & { log: CalculationLog }> => {
      ...
      try 
      {
        ...
        const lineItems = await relations.order.getLineItems({ orderId: fs.orderId });
        ...
        try
        {
           ...
           * 販売促進費
           */
          const campaignCapitalRate = await relations.campaignCapitalRate.getOne({ date: lineItems[0].orderedAt });
        ...
        try {
           ...
           */
          const claimsResult = await relations.claims.getByOrderId({ orderId: fs.orderId });
