...
import { CommissionFeeRate } from '../../amazon/constants';
...
export class FinalizedShippingConverter implements FirestoreDataConverter<IFinalizedShipping>
{
   ...
   */
  //-------------------------------------------------------------------------------
  fromCsvRow( tenant: Tenant, shippingDate: Date, row: IFinalizedShippingCsv, productMaster3: IProductMaster3[] ): IFinalizedShipping
  {
    // 指定日
    //  - 時間を0で統一
    //  - 日本時間に設定
    ...
    const date = new Date(shippingDate.toLocaleString('en-US', { timeZone: 'Asia/Tokyo' }))
    const dateAt = date.getTime();
    ...
    switch (tenant) 
    {
      ...
      case 'FBA': {
        ...
        //  - 日本時間に設定
        //  - UnixTimeに変換
        const f = new Date(row['purchase-date']);
        f.setHours(0, 0, 0, 0);
        ...
        const commissionFee = Number(found?.commission.replace('¥', '') || unitPrice * CommissionFeeRate) * quantity;
      ...
      case 'RSL': {
        ...
        return {
          ...
          promotionDiscount: row['店舗発行クーポン利用額'] || 0,
          commissionFee: 0,
          finalizedAt: finalizedAt,
      ...
      case 'SEIWA_YAHOO': {
        ...
        //  - 日本時間に設定
        //  - UnixTimeに変換
        const f = new Date(row['LeadTimeEnd']);
        f.setHours(0, 0, 0, 0);
        const finalizedDate = new Date(f.toLocaleString('en-US', { timeZone: 'Asia/Tokyo' }));
        const finalizedAt = finalizedDate.getTime();
      ...
      case 'SEIWA_QOO10': {
        ...
        const inventoryId = `${row['販売者コード']}-${row['販売者オプションコード']}`;
