 ...
 */
//-------------------------------------------------------------------------------
export const amazonCalculationTask: CalculationTask<IFinalizedSalesByOrder> = async (finalizedAt: Date) => {
  ...
  const amazonList = await finalizedShipping.getAll({
    finalizedAt: finalizedAt.getTime(),
    salesChannel: SalesChannel.Amazon,
  });

  const costCache = new Map<string, ICostMaster>();
  ...
  const result = await Promise.allSettled(
    ...
    amazonList.map(async (fs): Promise<IFinalizedSalesByOrder & { log: CalculationLog }> => {
      ...
      const product = await productMaster2.getOne({ inventoryId: fs.inventoryId, salesChannel: fs.salesChannel });
      if(product){
        
      
      const division = relations.divisionCodeMaster.find(
        (d) => d.brandName === product.brandName
      );
      ...
      if(!costCache.has(leoId)) {
        const cost = await costMaster.getOne({ leoId });
        if(cost){
          costCache.set(leoId, cost);
        }
      }
      const inventoryUnitPrice = costCache.get(leoId)?.costPrice || 0;
      ...
      try {
        const kpi = await relations.kpi.getOne({ inventoryId: fs.inventoryId, finalizedAt: fs.finalizedAt });
        if(kpi) {
